...
class MockProviderAdapter(BaseProviderAdapter):
    ...
    def build_provider_config(self, cfg):
        return {}
...
@pytest.mark.asyncio
async def test_full_pipeline_flow_end_to_end(mock_adapter):
    ...
    from pollux.core.types import FinalizedCommand, PlannedCommand
    ...
    with patch("pollux.pipeline.api_handler.APIHandler.handle") as mock_api:
        ...
        def side_effect(planned_cmd):
            ...
            return Success(FinalizedCommand(
                planned=planned_cmd,
                raw_api_response=mock_response_text
            ))
